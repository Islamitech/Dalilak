...
export const PackagesHub: React.FC<PackagesHubProps> = ({
  ...
  const [showEnterprise, setShowEnterprise] = useState<boolean>(false);
  ...
  const detailedPackages = [
    {
      ...
      featuresIncluded: [
        ...
        { name: 'هدية خاصة: ملصق باركود QR Code', desc: 'تصميم ملصق باركود مخصص لموقع نشاطك جاهز للطباعة والتعليق في المحل.' }
      ],
      idealPractices: [
  ...
  const getWhatsAppMessage = (pkg: typeof selectedPkg) => {
    return encodeURIComponent(
      `مرحباً دليلك 👋 أود الاستفسار والاشتراك في "${pkg.title}" بقيمة ${pkg.price} جنيه مصري لنشاطي التجاري.`
    );
  };
  ...
  return (
    <div className="space-y-6 font-['Cairo',sans-serif] text-[var(--text-primary)]">
        ...
        <div className="relative z-10 space-y-2.5">
          <div className="flex items-center gap-2">
            <span className="bg-slate-950/25 text-slate-950 text-[11px] sm:text-xs font-black px-3.5 py-1 rounded-full uppercase tracking-wider backdrop-blur-sm inline-flex items-center gap-1.5">
              <span>باقات التوثيق والتأسيس والنمو الرقمي</span>
              <span>💎</span>
            </span>
          </div>
        ...
        {detailedPackages.map((pkg) => {
          ...
          return (
            <div
              ...
              className={`p-5 sm:p-6 rounded-3xl border-2 transition-all duration-300 cursor-pointer flex flex-col justify-between space-y-5 shadow-sm hover:shadow-xl relative ${
                isSelected
            ...
            >
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${pkg.iconBg} flex items-center justify-center font-black shadow-md shrink-0`}>
                    <IconComp className="w-6 h-6 stroke-[2.5]" />
                  </div>
                  <span className={`text-[11px] sm:text-xs font-black px-3 py-1 rounded-full border ${pkg.badgeColor} shadow-sm`}>
                    {pkg.badge}
                  </span>
                </div>
                ...
                <div>
                  <h3 className="font-black text-lg sm:text-xl text-[var(--text-primary)] leading-snug">
                    {pkg.title}
                  </h3>
      ...
      {/* ========================================================================= */}
      <div className="bg-[var(--bg-card)] border-2 border-amber-500/30 rounded-3xl p-5 sm:p-7 space-y-6 shadow-xl transition-all duration-300">
          ...
          <div className="flex items-center gap-3.5">
            ...
            <div>
              ...
              <p className="text-xs sm:text-sm text-[var(--text-muted)] font-bold mt-1">
                الاستثمار: <span className="text-amber-500 font-black text-base">{selectedPkg.price} ج.م</span> | مدة التنفيذ: <span className="text-[var(--text-primary)] font-bold">{selectedPkg.deliveryTime}</span>
              </p>
            </div>
          </div>
          ...
          <div className="space-y-4 flex flex-col justify-between">
            ...
            <div className="pt-2 flex flex-col sm:flex-row items-center gap-3">
              <a
                ...
                className="w-full sm:flex-1 py-3.5 px-5 rounded-2xl font-black text-xs sm:text-sm bg-emerald-600 hover:bg-emerald-500 text-white shadow-lg shadow-emerald-600/20 flex items-center justify-center gap-2 transition-all active:scale-95 cursor-pointer"
              >
                <MessageCircle className="w-4 h-4" />
                <span>طلب هذه الباقة عبر واتساب فوراً 💬</span>
              </a>
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-3xl p-5 sm:p-7 space-y-4 shadow-sm transition-colors duration-300">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-right text-xs sm:text-sm border-collapse min-w-[650px]">
            <thead>
              <tr className="border-b border-[var(--border-color)] text-[var(--text-muted)]">
                <th className="py-3.5 px-4 font-black text-xs sm:text-sm text-[var(--text-primary)]">الخدمة / الميزة</th>
                <th className="py-3.5 px-3 font-black text-xs sm:text-sm text-center text-blue-600 dark:text-blue-400">
                  1. التوثيق الأساسي (250 ج)
                </th>
                <th className="py-3.5 px-3 font-black text-xs sm:text-sm text-center text-emerald-600 dark:text-emerald-400 bg-emerald-500/5 rounded-t-xl">
                  2. التأسيس والربط (750 ج) ⭐
                </th>
                <th className="py-3.5 px-3 font-black text-xs sm:text-sm text-center text-amber-600 dark:text-amber-400">
                ...
                <tr key={idx} className="hover:bg-[var(--bg-surface)] transition-colors">
                  ...
                  <td className="py-3.5 px-3 text-center">
                    {typeof row.basic === 'boolean' ? (
                      ...
                      <span className="font-extrabold text-blue-600 dark:text-blue-400 text-xs sm:text-sm">{row.basic}</span>
                    )}
                  </td>

                  {/* Pro */}
                  <td className="py-3.5 px-3 text-center bg-emerald-500/5 font-bold">
                    {typeof row.pro === 'boolean' ? (
                      ...
                      <span className="font-black text-emerald-700 dark:text-emerald-300 text-xs sm:text-sm">{row.pro}</span>
                    )}
                  </td>
                  ...
                  <td className="py-3.5 px-3 text-center font-bold">
                    {typeof row.vip === 'boolean' ? (
                      ...
                      <span className="font-black text-amber-700 dark:text-amber-400 text-xs sm:text-sm">{row.vip}</span>
                    )}
                  </td>
                </tr>
              ))}
      ...
      {/* ========================================================================= */}
      {/* 5. OPTIONAL TOGGLE: ENTERPRISE LAUNCH PACKAGE (20,000 EGP) */}
      {/* ========================================================================= */}
      <div className="pt-2">
        <button
          type="button"
          onClick={() => setShowEnterprise(!showEnterprise)}
          className="w-full py-4 px-6 rounded-3xl bg-gradient-to-r from-amber-500/10 via-[var(--bg-card)] to-yellow-500/10 border-2 border-amber-500/30 hover:border-amber-500 transition-all flex items-center justify-between text-right cursor-pointer group shadow-sm"
        >
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-gradient-to-r from-amber-500 to-yellow-500 text-slate-950 flex items-center justify-center font-black shadow-md">
              <Crown className="w-5 h-5" />
            </div>
            <div>
              <span className="font-black text-sm sm:text-base text-[var(--text-primary)] block">
                هل نشاطك تحت الإنشاء أو التأسيس من الصفر؟ 👑
              </span>
              <span className="text-xs text-amber-600 dark:text-amber-400 font-bold block mt-0.5">
                اضغط هنا لاستعراض باقة الانطلاق الكبرى والتأسيس الشامل (20,000 ج.م)
              </span>
            </div>
          </div>
          <span className="text-xs font-black text-amber-500 bg-amber-500/15 px-3 py-1.5 rounded-full border border-amber-500/30 group-hover:bg-amber-500 group-hover:text-slate-950 transition-colors">
            {showEnterprise ? 'إخفاء التفاصيل ▲' : 'استعراض الباقة ▼'}
          </span>
        </button>

        {showEnterprise && PACKAGES[4] && (
          <div className="mt-4 bg-gradient-to-br from-amber-500/15 via-[var(--bg-card)] to-yellow-500/10 border-2 border-amber-500 rounded-3xl p-6 sm:p-8 space-y-6 shadow-2xl relative overflow-hidden animate-fade-in">
            <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 border-b border-[var(--border-color)] pb-5">
              <div className="space-y-2">
                ...
                <p className="text-xs sm:text-sm text-[var(--text-secondary)] font-bold max-w-2xl leading-relaxed">
                  {PACKAGES[4].description}
                </p>
              </div>

              <div className="text-right lg:text-left shrink-0">
                ...
                <span className="text-[11px] text-[var(--text-muted)] font-bold block mt-1">
                  مشروع تأسيس متكامل من الصفر حتى الافتتاح
                </span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {PACKAGES[4].features.map((feat, i) => (
                <div key={i} className="flex items-start gap-2.5 p-2.5 bg-[var(--bg-card)] rounded-xl border border-[var(--border-color)] text-xs text-[var(--text-secondary)] font-bold">
            ...
            <div className="pt-2 flex flex-col sm:flex-row gap-3">
              <a
                href={`https://wa.me/201143888355?text=${encodeURIComponent(
                  `مرحباً دليلك 👋 أود الاستفسار عن باقة الانطلاق الكبرى والتأسيس من الصفر (20,000 ج.م) لنشاط جديد تحت الإنشاء.`
                )}`}
                target="_blank"
                rel="noopener noreferrer"
                className="flex-1 py-3.5 px-5 rounded-2xl font-black text-xs sm:text-sm bg-gradient-to-r from-amber-500 to-yellow-500 hover:from-amber-600 hover:to-yellow-600 text-slate-950 shadow-lg shadow-amber-500/20 flex items-center justify-center gap-2 transition-all active:scale-95 cursor-pointer"
              >
                <MessageCircle className="w-4 h-4" />
                <span>طلب استشارة تأسيس مجانية للباقة الكبرى عبر واتساب 💬</span>
              </a>

              {onSelectPackage && (
                <button
                  type="button"
                  onClick={() => handleSelectForForm(PACKAGES[4].title)}
                  className="py-3.5 px-5 rounded-2xl font-black text-xs sm:text-sm bg-[var(--input-bg)] hover:bg-slate-200 dark:hover:bg-slate-800 text-[var(--text-primary)] border border-[var(--border-color)] flex items-center justify-center gap-2 transition-all active:scale-95 cursor-pointer"
                >
