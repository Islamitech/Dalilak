...
function Preview() {
  ...
  return <main className="p-5 space-y-5" style={{ minHeight: 1200 }}>
    ...
    <button className="p-3 rounded-xl bg-amber-500" onClick={() => { setLegacy(true); setPhoto(true); }}>فتح الأصل والصورة معًا</button>
    ...
    {legacy && <OverlayLayer className="fixed inset-0 z-[100300] bg-slate-900/70 flex items-center justify-center p-4" onEscape={() => setLegacy(false)} onClick={() => setLegacy(false)}>
      <div className="bg-white p-6 rounded-xl space-y-4" style={{transform:'translateZ(0)'}} onClick={e => e.stopPropagation()}>
