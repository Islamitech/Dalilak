...
export const BusinessDetailsDrawer: React.FC<BusinessDetailsDrawerProps> = ({
  ...
  const userRole = currentUser?.role || 'rep';
  const userName = currentUser?.name || 'مستخدم النظام';
  const isGuest = (userRole as string) === 'guest';
  ...
  return (
    <>
      ...
      >
        ...
        {/* 3. Internal Navigation Tabs & Quick Edit Button */}
        {!isGuest && (
          <div className="flex items-center justify-between border-b border-slate-200 text-sm font-medium">
            ...
            {canEdit && onEditBusiness && (
              <button
                ...
                className="py-1 px-2.5 my-1 ml-2 rounded-xl bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-xs font-bold transition-all flex items-center gap-1.5 border border-indigo-200 shadow-2xs cursor-pointer whitespace-nowrap"
