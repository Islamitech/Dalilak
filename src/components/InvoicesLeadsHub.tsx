...
export const InvoicesLeadsHub: React.FC<InvoicesLeadsHubProps> = ({
  ...
  const generateWhatsAppMessage = (lead: InterestedLead, type: 'intro' | 'followup' | 'offer') => {
    const repName = currentUser?.name || currentRep?.name || 'فريق دليلك';
    const bizTitle = lead.businessName ? `نشاطكم "${lead.businessName}"` : 'نشاطكم التجاري';

    if (type === 'intro') {
      return `السلام عليكم ورحمة الله وبركاته يا فندم 👋
مع حضرتك ${repName} من منصة "دليلك" الرسمية لتوثيق الأنشطة التجارية على خرائط جوجل 🗺️✨

سعدت بلقاء حضرتك بخصوص ${bizTitle}.
نساعد نشاطكم في:
📍 التوثيق الميداني الرسمي الدائم على خرائط Google Maps باللوجو والصور وتفاصيل الاتصال.
🔍 تصدر نتائج البحث وظهور نشاطك لكل العملاء في ${lead.governorate} ومنطقتك.
🚀 باقة التوثيق الأساسي تبدأ من 250 جنيه فقط لتوثيق دائم للأبد بدون اشتراكات شهرية!

يشرفنا خدمتكم في أي وقت، وتقدر تتواصل معايا مباشرة عبر هذا الرقم 🤝`;
    }

    if (type === 'followup') {
      return `أهلاً بحضرتك يا فندم 🌹
معاك ${repName} من منصة دليلك.
بنتابع مع حضرتك بخصوص توثيق ${bizTitle} على خرائط جوجل.. هل تحب نحدد موعد مناسب لزيارة المندوب والبدء في رفع وتوثيق النشاط؟`;
    }

    // Offer
    return `عرض خاص لنشاطكم من منصة دليلك 🎁✨
احصل الآن على باقة التأسيس والربط الذكي لنشاط ${bizTitle} (توثيق خرائط جوجل + تأسيس المنصات وتصميم الإعلانات ومتابعة مستمرة) بسعر مميز جداً.
للتفاصيل وتأكيد الحجز، يرجى الرد على هذه الرسالة 🚀`;
  };
