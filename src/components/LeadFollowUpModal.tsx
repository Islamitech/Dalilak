...
const FOLLOW_UP_TYPE_LABELS: Record<AdminFollowUpType, { label: string; icon: string; color: string }> = {
  ...
  visit: { label: 'زيارة ميدانية', icon: '🚶‍♂️', color: 'bg-emerald-500/15 text-emerald-700 border-emerald-500/30' },
...
const FOLLOW_UP_STATUS_LABELS: Record<AdminFollowUpStatus, { label: string; color: string }> = {
  completed: { label: 'تم التنفيذ ✅', color: 'bg-emerald-500/20 text-emerald-700 border-emerald-500/40' },
  pending: { label: 'بانتظار الإجراء ⏳', color: 'bg-amber-500/20 text-amber-700 border-amber-500/40' },
  urgent: { label: 'عاجل وهام 🔥', color: 'bg-rose-500/20 text-rose-700 border-rose-500/40' },
};
...
export const LeadFollowUpModal: React.FC<LeadFollowUpModalProps> = ({
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-3 sm:p-5 overflow-y-auto">
      <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-3xl max-w-2xl w-full shadow-2xl flex flex-col my-auto max-h-[92vh] overflow-hidden text-[var(--text-primary)] animate-scale-up">
        ...
        <div className="bg-gradient-to-r from-emerald-500/10 via-[var(--bg-card)] to-teal-500/10 border-b border-[var(--border-color)] p-4 sm:p-5 flex items-start justify-between gap-3 shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-2xl bg-emerald-500/20 text-emerald-600 flex items-center justify-center font-bold shadow-inner shrink-0">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {currentLead.status === 'converted' && (
                  <span className="bg-emerald-500/15 text-emerald-700 text-[11px] font-black px-2.5 py-0.5 rounded-full border border-emerald-500/30">
                    تم التحويل لاشتراك 🌟
                  </span>
                )}
                {currentLead.status === 'contacted' && (
                  <span className="bg-blue-500/15 text-blue-700 text-[11px] font-black px-2.5 py-0.5 rounded-full border border-blue-500/30">
                    تم التواصل 📞
                  </span>
                )}
                {currentLead.status === 'pending_followup' && (
                  <span className="bg-amber-500/15 text-amber-700 text-[11px] font-black px-2.5 py-0.5 rounded-full border border-amber-500/30">
                    بانتظار المتابعة ⏳
                  </span>
                )}
              </div>
        ...
        <div className="p-4 sm:p-6 overflow-y-auto space-y-5 text-xs">
          {notification && (
            <div className="bg-emerald-500/15 border border-emerald-500/30 text-emerald-700 px-4 py-2.5 rounded-2xl font-bold flex items-center gap-2 animate-fade-in">
                  ...
                  {extractedMapUrl && (
                    <div className="flex items-center gap-2 flex-wrap pt-1">
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-blue-500/10 hover:bg-blue-500/20 border border-blue-500/25 text-blue-700 font-bold text-xs transition-colors"
                      ...
                      <a
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/25 text-amber-700 font-bold text-xs transition-colors"
                      >
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-[var(--border-color)]">
              <div className="flex items-center gap-2">
                <a
                  ...
                  className="px-3 py-1.5 rounded-xl bg-blue-500/10 hover:bg-blue-500/20 text-blue-700 border border-blue-500/25 font-bold flex items-center gap-1.5 transition-colors cursor-pointer"
                >
                ...
                {waUrl && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="px-3 py-1.5 rounded-xl bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-700 border border-emerald-500/25 font-bold flex items-center gap-1.5 transition-colors cursor-pointer"
                  >
                ...
                {mapUrl && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="px-3 py-1.5 rounded-xl bg-amber-500/10 hover:bg-amber-500/20 text-amber-700 border border-amber-500/25 font-bold flex items-center gap-1.5 transition-colors cursor-pointer"
                  >
                    <MapPin className="w-3.5 h-3.5" />
          ...
          <div className="border border-emerald-500/30 rounded-2xl p-4 sm:p-5 bg-gradient-to-br from-emerald-500/5 via-[var(--bg-card)] to-teal-500/5 space-y-4">
            <div className="flex items-center justify-between border-b border-[var(--border-color)] pb-2.5">
              <h4 className="font-black text-sm text-[var(--text-primary)] flex items-center gap-2">
                ...
                <span className="bg-emerald-500/20 text-emerald-700 text-[10px] font-mono px-2 py-0.5 rounded-full">
        ...
        <div className="p-3 sm:p-4 bg-[var(--input-bg)] border-t border-[var(--border-color)] flex items-center justify-between gap-3 shrink-0">
          ...
          <button
            type="button"
            onClick={onClose}
            className="px-5 py-2 rounded-xl bg-slate-200 hover:bg-slate-300 text-[var(--text-primary)] font-bold text-xs transition-colors cursor-pointer"
          >
            إغلاق
          </button>
        </div>
