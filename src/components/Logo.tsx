...
export const Logo: React.FC<LogoProps> = ({
  ...
  const IconElement = (
    <div className={`relative ${iconDimensions} shrink-0 group cursor-pointer select-none`}>
      {/* Ambient background glow */}
      <div className="absolute inset-0 rounded-2xl bg-cyan-500/20 dark:bg-amber-500/25 blur-sm group-hover:blur-md transition-all duration-300" />
      
      {/* Clean high-resolution 3D Emblem Asset */}
      <img
        src="/logo.png"
        alt="دليلك - المنصة الرسمية لتوثيق الأنشطة التجارية"
        className="relative w-full h-full object-contain rounded-2xl shadow-md border border-slate-700/20 dark:border-amber-500/30 transform group-hover:scale-105 transition-transform duration-300"
      />
    </div>
  );
    ...
    <div className={`flex items-center gap-2.5 sm:gap-3.5 select-none group ${className}`}>
      ...
      <div className="flex flex-col justify-center text-right">
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          <span className="hidden sm:inline-flex items-center gap-1 bg-amber-500/15 text-amber-700 dark:text-amber-300 text-[9px] font-black px-2 py-0.5 rounded-full border border-amber-500/30">
            خرائط جوجل 🇪🇬
          </span>
        </div>
