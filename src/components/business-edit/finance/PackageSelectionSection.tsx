...
import { ShieldCheck, Sparkles, DollarSign, CheckCircle2, Clock, CreditCard } from 'lucide-react';

export interface PackageSelectionSectionProps {
  formData: Business;
...
export const PackageSelectionSection: React.FC<PackageSelectionSectionProps> = ({
  formData,
  setFormData,
  isEditMode,
  isAdminOrFinancial,
      ...
      {!isAdminOrFinancial && (
        <div className="bg-sky-500/10 border border-sky-500/30 text-sky-800 p-3.5 rounded-2xl text-xs font-bold leading-relaxed flex items-center gap-2.5 shadow-2xs">
          ...
          <span>
            ...
            ويُدار حصرياً من قِبل إدارة المنظومة ومسؤولي الحسابات.
          </span>
        </div>
      )}
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="text-right">
                ...
                <p className="text-[11px] text-[var(--text-muted)] font-bold mt-0.5">
                  إزالة التحصيل وتصفير الفاتورة (0 ج.م) واستبعاد النشاط وفواتيره تماماً من الإحصائيات
                  والديون
                </p>
              </div>
            </div>
              ...
              >
                <span
                  className={`w-2.5 h-2.5 rounded-full ${
                    formData.isFeeExempt ? 'bg-white animate-pulse' : 'bg-slate-400'
                  }`}
                />
                <span>
                  {formData.isFeeExempt
                    ? '✓ نشاط معفى (إدراج مجاني)'
                    : 'نشاط تجاري عادي (اضغط للإعفاء)'}
                </span>
              </button>
            ) : (
              <span
        ...
        <div className="bg-[var(--input-bg)] border border-[var(--border-color)] rounded-2xl p-3 space-y-1">
          ...
          {isEditMode && isAdminOrFinancial ? (
            <select
              value={
                formData.packageId ||
                (formData.isAlreadyOnGoogle ? ALREADY_ON_GOOGLE_PACKAGE.id : PACKAGES[0].id)
              }
              onChange={(e) => {
      ...
      <div className="bg-[var(--input-bg)] border border-[var(--border-color)] rounded-2xl p-3.5 space-y-2">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          ...
          {isEditMode && isAdminOrFinancial && !formData.isFeeExempt ? (
            ...
            >
              <option value="platform_collected">
                تحويل مباشر لحسابات المنصة (إلكتروني / إنستاباي / فودافون كاش)
              </option>
