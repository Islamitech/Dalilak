...
export const DirectoryMetricsBar: React.FC<DirectoryMetricsBarProps> = ({
  ...
  return (
    <div className={`grid ${isRep ? 'grid-cols-4' : 'grid-cols-6 sm:grid-cols-5'} gap-1.5 sm:gap-2.5`}>
      {/* 1. إجمالي الأنشطة المسجلة */}
      <div className={`${isRep ? 'col-span-2 sm:col-span-1' : 'col-span-2 sm:col-span-1'} bg-[var(--bg-card)] border border-[var(--border-color)] p-2 sm:p-2.5 rounded-xl sm:rounded-2xl shadow-2xs flex flex-col items-center justify-center text-center transition-all min-w-0`}>
      ...
      {(!isRep || stats.directoryApproved > 0) && (
        <div className={`${isRep ? 'col-span-2 sm:col-span-1' : 'col-span-2 sm:col-span-1'} bg-[var(--bg-card)] border border-emerald-500/30 p-2 sm:p-2.5 rounded-xl sm:rounded-2xl shadow-2xs flex flex-col items-center justify-center text-center transition-all min-w-0 animate-fade-in`}>
      ...
      {(!isRep || stats.googleMapsVerified > 0) && (
        <div className={`${isRep ? 'col-span-2 sm:col-span-1' : 'col-span-2 sm:col-span-1'} bg-[var(--bg-card)] border border-blue-500/30 p-2 sm:p-2.5 rounded-xl sm:rounded-2xl shadow-2xs flex flex-col items-center justify-center text-center transition-all min-w-0 animate-fade-in`}>
      ...
      {(!isRep || hasRegisteredBiz) && (
        <div className={`${isRep ? 'col-span-2 sm:col-span-1' : 'col-span-3 sm:col-span-1'} bg-[var(--bg-card)] border border-amber-500/30 p-2 sm:p-2.5 rounded-xl sm:rounded-2xl shadow-2xs flex flex-col items-center justify-center text-center transition-all min-w-0 animate-fade-in`}>
      ...
      {/* 5. تغطية المحافظات */}
      {!isRep && (
        <div className="col-span-3 sm:col-span-1 bg-[var(--bg-card)] border border-purple-500/30 p-2 sm:p-2.5 rounded-xl sm:rounded-2xl shadow-2xs flex flex-col items-center justify-center text-center transition-all min-w-0">
