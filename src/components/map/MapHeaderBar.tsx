...
import { MapPin, Sparkles, Loader2, Navigation, Maximize2, Minimize2, Layers, Globe } from 'lucide-react';
    ...
    <div className="bg-[var(--map-header-bg)] p-2.5 sm:p-3 border-b border-[var(--map-header-border)] flex flex-wrap items-center justify-between gap-2 z-20">
      ...
      <div className="flex items-center gap-1.5 flex-wrap">
        ...
        <div className="flex items-center bg-[var(--input-bg)] p-0.5 rounded-xl border border-[var(--border-color)] text-[11px] font-bold">
          <button
            type="button"
            onClick={() => onSwitchTileLayer('dalelak-osm')}
            className={`px-2.5 py-1 rounded-lg transition-all flex items-center gap-1 cursor-pointer ${
              tileLayer === 'dalelak-osm'
                ? 'bg-amber-500 text-slate-950 font-black shadow'
                : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]'
            }`}
            title="عرض خريطة دليلك الميدانية الرسمية (OpenStreetMap)"
          >
            <Layers className="w-3 h-3" />
            <span>خريطة دليلك الرسمية</span>
          </button>
          <button
            type="button"
