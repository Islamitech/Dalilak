...
export const InteractiveOnboardingExperience: React.FC<InteractiveOnboardingExperienceProps> = ({ onExploreAround, onSearchSpecific, onAddBusinessFree, onSkip }) => {
  ...
  return (
    <div className={`fixed inset-0 z-[70] overflow-y-auto bg-[var(--bg-primary)] text-[var(--text-primary)] transition-all duration-300 ${isExiting ? 'pointer-events-none opacity-0' : 'opacity-100'}`}>
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-6xl flex-col px-4 py-5 sm:px-6 sm:py-7">
        ...
        <main className="mx-auto grid w-full max-w-5xl flex-1 items-center gap-6 py-6 lg:grid-cols-[1.04fr_.96fr] lg:py-10">
          <section className="order-2 rounded-[2rem] border border-[var(--border-color)] bg-[var(--bg-card)] p-5 shadow-xl shadow-slate-900/[.04] sm:p-7 lg:order-1">
            ...
            <h1 className="mt-3 text-2xl font-black tracking-tight text-[var(--text-primary)] sm:text-3xl">ما الذي تبحث عنه اليوم؟</h1><p className="mt-2 max-w-lg text-sm font-bold leading-6 text-[var(--text-muted)]">اختر وجهتك وسنفتح الدليل بالطريقة الأنسب لك فوراً.</p>
            <div className="mt-6 grid gap-3 sm:grid-cols-3">
              ...
              <button type="button" onClick={() => handleAction('search')} className={`rounded-2xl border p-4 text-right transition hover:-translate-y-0.5 focus:outline-none focus:ring-4 focus:ring-slate-100 cursor-pointer ${selectedIntent === 'search' ? 'border-slate-400 bg-slate-100' : 'border-[var(--border-color)] bg-slate-50 hover:border-slate-300 hover:bg-[var(--bg-card)]'}`}><span className="mb-5 flex h-9 w-9 items-center justify-center rounded-xl bg-slate-900 text-white"><Search className="h-4 w-4" /></span><strong className="block text-sm font-black text-slate-900">ابحث عن خدمة</strong><span className="mt-1 block text-[11px] font-bold leading-5 text-slate-500">بالاسم أو التصنيف</span></button>
              <button type="button" onClick={() => handleAction('add')} className={`rounded-2xl border p-4 text-right transition hover:-translate-y-0.5 focus:outline-none focus:ring-4 focus:ring-emerald-100 cursor-pointer ${selectedIntent === 'add' ? 'border-emerald-500 bg-emerald-100' : 'border-emerald-200 bg-emerald-50 hover:emerald-100'}`}><span className="mb-5 flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-600 text-white"><Store className="h-4 w-4" /></span><strong className="block text-sm font-black text-slate-900">أضف نشاطك</strong><span className="mt-1 block text-[11px] font-bold leading-5 text-slate-500">ابدأ ظهورك مجاناً</span></button>
            </div>
          </section>

          <section className="order-1 rounded-[2rem] border border-amber-200 bg-[var(--bg-card)] p-4 shadow-xl shadow-amber-900/[.04] sm:p-6 lg:order-2">
            <div className="mb-3 flex items-start justify-between"><div><h2 className="text-base font-black text-[var(--text-primary)]">استكشف على خريطة مصر</h2><p className="mt-1 text-[11px] font-bold text-[var(--text-muted)]">اختر مدينة للبدء — جميع النقاط قابلة للضغط</p></div><MapPin className="h-5 w-5 text-amber-500" /></div>
            <div className="relative mx-auto aspect-[1.12/1] max-w-md overflow-hidden rounded-3xl border border-[var(--border-color)] bg-[linear-gradient(145deg,#f8fcff,#ecf4f6)]">
