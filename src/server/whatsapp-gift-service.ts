...
export interface GiftPackageOptions {
  hidePhone?: boolean;
  customPhone?: string;
 ...
 */
export async function findCompanionAppBundle(
  bizId: string,
  biz: Business,
  options?: GiftPackageOptions
): Promise<BusinessGiftItem[]> {
  ensureDirectories();
  ...
  const needsFreshRender = Boolean(options?.hidePhone || options?.customPhone || options?.forceRegenerate);
  ...
  // If bundle is incomplete (< 4) or fresh customized render requested, auto-invoke QR Booster
  if (foundItems.length < 4 || needsFreshRender) {
    ...
    if (fs.existsSync(qrBoosterScript)) {
      try {
        let renderCmd = `cmd /c npx.cmd tsx scripts/render_bundle.ts "${bizId}"`;
        if (options?.hidePhone) {
          renderCmd += ' --hide-phone';
        }
        if (options?.customPhone) {
          const sanitized = options.customPhone.replace(/["'\\]/g, '').trim();
          renderCmd += ` --custom-phone "${sanitized}"`;
        }

        console.log(`[Gift Service] Invoking QR Booster render for ${bizId} (needsFresh: ${needsFreshRender}, hidePhone: ${Boolean(options?.hidePhone)}, customPhone: ${options?.customPhone || 'none'})...`);
        await new Promise<void>((resolve) => {
          exec(
            renderCmd,
            { cwd: qrBoosterCwd, timeout: 35000 },
            (err, stdout, _stderr) => {
              if (err) {
...
export async function prepareBusinessGiftPackage(
  biz: Business,
  options?: GiftPackageOptions
): Promise<BusinessGiftResult> {
  ensureDirectories();
  const venueName = biz.nameAr || biz.name || 'المنشأة الكريمة';
  const directoryUrl = getDisplayDirectoryUrl(biz);

  // 1. Try finding complete companion app bundle (4 designs)
  const bundle = await findCompanionAppBundle(biz.id, biz, options);
